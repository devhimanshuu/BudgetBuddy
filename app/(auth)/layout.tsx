...
const layout = ({ children }: { children: ReactNode }) => {
  return (
    <div className="relative flex min-h-screen w-full flex-col items-center overflow-y-auto overflow-x-hidden bg-background py-8 sm:py-12">
      ...
      <div className="fixed bottom-32 right-16 opacity-20 animate-float hidden lg:block" style={{ animationDelay: '1s' }}>
        <PieChart className="w-14 h-14 text-emerald-500" />
      </div>
      <div className="fixed top-1/3 right-10 opacity-15 animate-float hidden lg:block" style={{ animationDelay: '2s' }}>
        <TrendingUp className="w-16 h-16 text-blue-500" />
      </div>
      <div className="fixed bottom-20 left-20 opacity-15 animate-float hidden lg:block" style={{ animationDelay: '3s' }}>
